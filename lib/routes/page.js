module.exports = function(crowi, app) {
  ...
  /**
   * @api {post} /pages.isCreatable Check whether given page is creatable or not
   * @apiName isCreatable
   * @apiGroup Page
   *
   * @apiParam {String} path
   */
  api.isCreatable = function(req, res){
    var path = req.body.path;

    if (!Page.isCreatableName(path)) {
      return res.json(ApiResponse.error(sprintf('このページ名は作成できません (%s)', path)));
    }

    Page.findPageByPath(path)
    .then(function(page) {
      // if page found, cannot cannot rename to that path
      return res.json(ApiResponse.error(sprintf('このページ名は作成できません (%s)。ページが存在します。', path)));
    }).catch(function(err) {
      var result = {};
      return res.json(ApiResponse.success(result));
    });
